...
namespace processor
{
    ...
    {
        if (address1 > sizeof(InternalMemory.memory) || address2 > sizeof(InternalMemory.memory)){throw CPUStop_OutOfMemoryRange();}
        ...
        if (address2 == NULL && address2_t != NULL && _mbstrlen(address2_t) > 1) {
            for (int x = 0; x < _mbstrlen(address2_t); x++) {
    ...
    void add(int address1, int address2)
    {
        ...
        if (InternalMemory.memory[address1] > 65536 || InternalMemory.memory[address2] > 65536){throw CPUCritical_MathExecutionIncorrect();}
        if (isdigit(InternalMemory.memory[address1]) != true || isdigit(InternalMemory.memory[address2]) != true){throw CPUError_IsNotAvalue();}
        if (InternalMemory.memory[address1] + InternalMemory.memory[address2] > 65536){throw CPUCritical_MathExecutionIncorrect();}
    ...
    {
        if (address1 > sizeof(InternalMemory.memory) || address2 > sizeof(InternalMemory.memory)){throw CPUStop_OutOfMemoryRange();}
        if (InternalMemory.memory[address1] == '0' || InternalMemory.memory[address2] == '0'){throw CPUCriticalError_DivideByZero();}
        if (address1 < 0 || address2 < 0){throw CPUStop_OutOfMemoryRange();}
        if (InternalMemory.memory[address1] > 65536 || InternalMemory.memory[address2] > 65536){throw CPUCritical_MathExecutionIncorrect();}
        if (isdigit(InternalMemory.memory[address1]) != true || isdigit(InternalMemory.memory[address2]) != true){throw CPUError_IsNotAvalue();}
        if (InternalMemory.memory[address1] + InternalMemory.memory[address2] > 65536){throw CPUCritical_MathExecutionIncorrect();}
    ...
    void cmp(int address1, int address2)
    {
        ...
        if (address1 < 0 || address2 < 0){throw CPUStop_OutOfMemoryRange();}
        if (InternalMemory.memory[address1] > 65536 || InternalMemory.memory[address2] > 65536){throw CPUCritical_MathExecutionIncorrect();}
        if (InternalMemory.memory[address2] == InternalMemory.memory[address1]){InternalMemory.memory[3] = '1';} else {InternalMemory.memory[3] = '0';}
    }
